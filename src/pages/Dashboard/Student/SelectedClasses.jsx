  ...
  return (
    <>
      <div className="alert shadow-lg w-full rounded-none ">
        ...
        <div className="px-4">
          <h3 className="font-bold">Total Selected Class : {data?.length}</h3>
          <div className="text-base font-semibold">Totel Price: ${price}</div>
        </div>
      </div>
      <section className="container mx-auto py-8">
        <div className="flex gap-4 flex-wrap items-center justify-center">
          {data?.map((item) => (
            <SelectedItem key={item?._id} item={item} />
          ))}
        </div>
      </section>
    </>
